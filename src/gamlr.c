...
double ysum,ybar,wysum;
double *H = NULL;
double *G = NULL;
double *ag0 = NULL;

// function pointers
double (*nllhd)(int, double, double , double*, double*, double*, double*) = NULL;
double (*reweight)(int, double, double, double*,
        double *, double*, double*, int*, double*) = NULL;
...
void dograd(int j){
  ...
  if(prexx){
    ...
    int i;
    for(i=xp[j]; i<xp[j+1]; i++)
      G[j] += V[xi[i]]*xv[i]*(E[xi[i]]+log(tocounts[xi[i]]));
  }
}
...
int cdsolve(double tol, int M, int RW)
{
  ...
  while( (Bdiff > tol) | dozero ){
    ...
    if(dozero){
      if( (fam!=1) & (RW>rw) ){
        rw +=1;
        vsum = reweight(n, A, alphanb, E, Y, V, Z, &exitstat,tocounts);
        docurve();
        dbet = intercept(n, E, V, Z, vsum,tocounts)-A;
        A += dbet;
...
void R_gamlr(int *famid, // 1 gaus, 2 bin, 3 pois
             ...
             double *alphanb_in, //parameter to  adjusts the variance of the negative binomial distribution
             double *tocounts_in, //the y count adds up
             ...
             int *verb) // talk?
{
  ...
  case 4:
    nllhd = &nb_nllhd;
    reweight = &nb_reweight;
    A = log(ybar);
    // nonzero saturated negative log likelihood
    wysum=0;
    for(int i=0;i<n;i++){
      wysum += (1/alphanb+Y[i])*log(1+alphanb*Y[i]);
    }
    NLsat = wysum;
    for(int i=0; i<n; i++){
      if(Y[i]!=0) NLsat += -Y[i]*log(alphanb*Y[i]);
    }
      break;
  default:
    ...
    nllhd = &sse;
    NLsat=0.0;
    A = intercept(n, E, V, Z, vsum,tocounts);
    int j;
    for(j=0; j<p; j++) dograd(j);
  }

  NLLHD =  nllhd(n, A, alphanb, E, Y, V, tocounts);
  if(*verb)
