...
cv.gamlr <- function(x, y, nfold=5, foldid=NULL, verb=FALSE, cl=NULL, ...){
  ...
  alphanb=argl$alphanb
  tocounts=argl$tocounts
  ...
  folddev <- function(k){
    ...
    eta <- predict(fit, x[-train,,drop=FALSE],tocounts=tocounts[-train], family=fam,select=0)
    ...
    dev <- apply(eta,2,
                 function(e)
                   mean(switch(fam,
                               ...
                               "negative binomial" = -2*(y[-train]*e - (1/alphanb+y[-train])*log(1+alphanb*exp(e))))))
    ...
    if(fam=="negative binomial"){
      satnllhd <- mean(ifelse(y[-train]>0,
                              y[-train]*log(y[-train]),
                              0.0) - (1/alphanb+y[-train])*log(1+alphanb*y[-train]))
      dev <- dev + 2*satnllhd }
...
predict.cv.gamlr <- function(object, newdata, tocounts,family,
                             select=c("1se","min"), ...){
  seg = paste("seg",match.arg(select),sep=".")
  predict.gamlr(object$gamlr, newdata, tocounts, family=family,select=object[[seg]], ...)
}
