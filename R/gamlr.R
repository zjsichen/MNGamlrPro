...
gamlr <- function(x, y,
                  family=c("gaussian","binomial","poisson","negative binomial"),
                  gamma=0,
                  alphanb=1,
                  tocounts=NULL,
                  nlambda=100,
                  ...
                  verb=FALSE, ...)
{
  ...
  famid = switch(family,
                 "gaussian"=1, "binomial"=2, "poisson"=3,"negative binomial"=4)

  ## data checking (more follows)
  n <- length(y)
  if(is.null(tocounts)){
    if(family!="gaussian") tocounts=y else tocounts=rep(1,n)
  }
  y <- checky(y,family)
  ...
  on.exit(.C("R_gamlr_cleanup", PACKAGE = "MNGamlrPro"))
  fit <- .C("R_gamlr",
...
predict.gamlr <- function(object, newdata,tocounts,family,
                          type = c("link", "response"), ...)
{
  ...
  if(family!="gaussian") eta=eta+matrix(log(tocounts),ncol = 1)
  ...
  if(object$family=="negative binomial" & type=="response")
    return(apply(eta,2,exp))
